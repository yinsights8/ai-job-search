...
| Candidate profile | Step 1 (.opencode/skills/job-application-assistant/01-candidate-profile.md) | Yes |
| Rewrite prompt (4-stage) | Read `personalised/rewrite-cv.md` | **No — read this now** |
| Template structure | Read `personalised/main.tex` | **No — read this now** |

### 7b. Generate Personalised CV

First run the **4-stage process from `personalised/rewrite-cv.md`** (Stage 1 CV analysis →
Stage 2 JD analysis → Stage 3 fit score & gap analysis → Stage 4 final CV content), using
the job posting from Step 0 and the generated CV content from Step 2/4 as its two inputs.
Stages 1-2 may reuse analysis already done in Step 1. Present the Stage 3 fit score and
top gaps to the user in chat, apply only **Quick Win** changes to the CV content, and
report **Needs Effort** items in chat — never embed `[Add this once you complete X]`
placeholders in the LaTeX output.

Then typeset the Stage 4 content as `personalised/cv_<company>.tex` using the
**article-class template** from `personalised/main.tex` as the structural reference.
Use `pdflatex` (not lualatex).
...
\section*{SKILLS}
% Skill categories, itemize with leftmargin=*, nosep
...
|---------------------------|-----------|
| UK Data Job Market Dashboard (Power BI) | `https://github.com/your-github/example-project-1` |
| Archival RAG System | `https://github.com/your-github/example-project-2` |
| Violence Detection using Deep Learning | `https://github.com/your-github/example-project-3` |
| Restaurant Ratings Analysis (Power BI) | `https://github.com/your-github/example-project-4` |

#### Formatting rules to apply (template conventions)

The analysis process comes from `personalised/rewrite-cv.md`; these formatting rules
match `personalised/main.tex`. Apply ALL of the following while typesetting:
