...
1. **The archive** (maintained by `/outcome`): `documents/applications/<company>_<role>/`
   ...
   - `emails/_index.md` and `emails/<stem>.md` (if present, populated by `/scan-email`) - read the prior emails in the thread for explicit weaknesses the candidate should address. If a prior rejection email mentioned a specific gap, that gap should be addressed or mitigated in the next application. If the recruiter's tone shifted (cold → warm), that's a signal of where the candidate is in their pipeline.
